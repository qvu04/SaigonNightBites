  ...
  return (
    ...
    >
      <div className="relative h-40 bg-gradient-to-br from-orange-100 to-orange-200 dark:from-zinc-700 dark:to-zinc-600">
        {photo_url ? (
          <img src={photo_url} alt={name} className="w-full h-full object-cover" loading="lazy" />
        ) : (
        ...
        >
          <FiHeart className={clsx('w-4 h-4 transition-colors', isFavorited ? 'fill-red-500 stroke-red-500' : 'stroke-zinc-400 dark:stroke-zinc-500')} />
        </button>
      </div>
        ...
        <div className="flex items-center gap-3 mb-1.5">
          ...
          {distance != null && (
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-zinc-500 dark:text-zinc-400">{formatDistance(distance)}</span>
            </div>
          )}
        </div>
